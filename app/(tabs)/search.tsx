...
import { useKDramas, usePopularMovies, usePopularTV, useSearch } from '@/hooks/use-tmdb';
import { formatRating, GENRE_MAP, getTitle, getYear, posterUrl, TMDBItem } from '@/lib/tmdb';
...
function ResultRow({ item, onPress }: { item: TMDBItem; onPress: () => void }) {
  ...
  const genres = item.genre_ids.slice(0, 2).map(id => GENRE_MAP[id]).filter(Boolean).join(', ');
  const rating = formatRating(item.vote_average);

  return (
    <TouchableOpacity style={styles.resultItem} onPress={onPress} activeOpacity={0.8}>
      {poster ? (
        <Image source={{ uri: poster }} style={styles.resultThumb} contentFit="cover" transition={200} />
      ) : (
      ...
      <View style={styles.resultInfo}>
        ...
        <Text style={styles.resultMeta}>{type}{year ? ` · ${year}` : ''}{genres ? ` · ${genres}` : ''}</Text>
      </View>
...
export default function SearchScreen() {
  ...
  const handlePress = (item: TMDBItem) => {
    ...
    router.push({ pathname: '/details', params: { id: String(item.id), type } });
  };
      ...
      <View style={styles.searchRow}>
        <View style={styles.searchBox}>
          ...
          <TextInput
            ...
            placeholder="Search movies, series, kdramas..."
      ...
      {isSearching ? (
        <View style={styles.resultsContainer}>
          {searchResults.loading ? (
            ...
            <Text style={styles.errorText}>Could not load results. Check your API token.</Text>
            ...
            <FlatList
              data={searchResults.data?.results.filter(
                i => (i as any).media_type === 'movie' || (i as any).media_type === 'tv'
              )}
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
          ...
          {/* Categories */}
          <Text style={[styles.sectionTitle, { marginTop: Spacing.lg }]}>Categories</Text>
          ...
          {kdramas.loading ? (
            ...
            kdramas.data?.results.slice(0, 6).map(item => (
              <ResultRow
                ...
                onPress={() => router.push({ pathname: '/details', params: { id: String(item.id), type: 'tv' } })}
              />
            ))
          )}
...
const styles = StyleSheet.create({
  ...
  resultInfo: { flex: 1 },
  resultTitle: { ...TextStyles.titleSmall, color: Colors.primaryText, marginBottom: 3 },
  ...
  emptySearch: { alignItems: 'center', paddingTop: Spacing.xxxl, gap: Spacing.sm },
  emptyText: { ...TextStyles.bodyMedium, color: Colors.hint },
  errorText: { ...TextStyles.bodyMedium, color: Colors.error, textAlign: 'center', padding: Spacing.lg },

  // Browse
  scroll: { paddingHorizontal: Spacing.lg },
  sectionTitle: { ...TextStyles.titleMedium, color: Colors.primaryText, marginBottom: Spacing.md },
  ...
  tagText: { ...TextStyles.bodySmall, color: Colors.primaryText, flexShrink: 1 },
  catGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: Spacing.md },
  catCard: {
    width: '47%', height: 80, borderRadius: Radii.lg,
